...
export function cardCartProductComponent(
  ...
  return `
    ...
    product.title
        ...
        <p class="font-livvic text-white text-base my-2">Quantidade: ${
          productCart.quantity
        }</p>
        <button id="remove-from-cart-${
          product.id
        }" class="mt-auto flex justify-center py-2 px-4 items-center bg-ada_green font-livvic rounded-lg text-ada_navyblue-100">Remover do Carrinho</button>
      </div>
    `;
}
