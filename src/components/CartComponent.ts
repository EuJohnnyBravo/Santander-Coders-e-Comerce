...
import { addProductToCart, clearCart, getSessionUserCart, removeProductCart } from "../services/cartService";
...
const sectionCheckout = document.getElementById("section-checkout") as HTMLElement;
...
export function loadCardCartComponents(products: IProduct[]){
...
async function checkout(){
  ...
  const notifyCheckout = document.getElementById("notify-checkout") as HTMLElement;
  if(productsCart.length > 0){
    clearCart();
    await updateComponents();
    if(notifyCheckout) notifyCheckout.innerHTML = `<p class="text-center text-ada_green text-xl font-livvic">Compra Realizada com Sucesso!</p>`;
    return;
  }
  if(notifyCheckout) notifyCheckout.innerHTML = `<p class="text-center text-red-500 text-xl font-livvic">Nenhum Produto no carrinho!</p>`;
  return;
}
